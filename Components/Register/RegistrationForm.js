...
function RegistrationForm({email, password, repassword, setEmail, setPassword, setRePassword, validFom, handleSubmit, code, loading}) {
  return (
    <>
      <form onSubmit={(e) => handleSubmit(e)} className='flex flex-col gap-3 mx-2'>
      ...
      <button style={{borderColor:'rgba(0, 0, 0, 0.27)'}} className=' border rounded-md w-full cursor-pointer my-3 flex justify-center py-2 mx-2'> <div className=' flex flex-row gap-4'><Image width={25} height={25} src={'https://bucket-tkc.s3.ap-south-1.amazonaws.com/google.png'} alt='logo google'/>{'  '} <p className=' text-[16px]'><Link href='https://tkcapi.tsasoft.com/auth/google' passHref legacyBehavior>Continue with Google</Link></p></div></button>
