...
function RegistrationForm({email, password, repassword, setEmail, setPassword, setRePassword, validFom, handleSubmit}) {
  return (
    <>
      <form onSubmit={(e) => handleSubmit(e)} className='flex flex-col gap-3'>
        ...
        <div className = 'flex justify-center'><button type='submit' className={`text-white font-semibold w-full p-2 rounded-md cursor-pointer`} style={{backgroundColor:validFom?'rgba(7, 60, 160, 1)':'rgba(101, 137, 204, 1)'}} disabled={!validFom}>Register</button></div>
      </form>
