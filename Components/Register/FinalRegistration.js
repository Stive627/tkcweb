...
const EnterPassword = ({password, setpassword, handleNext, passwordRef}) => {
    return(
        <div ref={passwordRef}>
            ...
            <TkcInput  className={'w-96 my-2.5'} value={password} handleChange={setpassword} placeholder={'Enter your password'}/>
            <p className='text-[11px]'>The password must contain at least six characters, a number, and a capital letter.</p>
            <div className=' flex justify-center'><button onClick={()=> handleNext()} className=' text-white font-semibold w-1/3 p-2 rounded-md cursor-pointer' style={{backgroundColor:'rgba(7, 60, 160, 1)'}} disabled={false}>Continue</button></div>
        </div>
    )
}
...
const VerifyPassword = ({repassword, setRepassword, repasswordRef, handleNext}) => {
    return(
        <div ref={repasswordRef}>
            ...
            <TkcInput className={'w-96 my-1.5'}  value={repassword} handleChange={setRepassword} placeholder={'Enter your password'}/>
            <p className='text-[11px]'>The password must be the one entered previously.</p>
            <div className=' flex justify-center mt-2'><button onClick={()=> handleNext()} className=' text-white font-semibold w-1/3 p-2 rounded-md cursor-pointer' style={{backgroundColor:'rgba(7, 60, 160, 1)'}} disabled={false}>Continue</button></div>
        </div>
    )
}
...
const UsernameUX = ({username, setUsername, usernameRef, handleNext}) => {
    return(
        <div ref={usernameRef}>
            ...
            <TkcInput className={'w-96 my-1.5'}  value={username} handleChange={setUsername} placeholder={'Enter your username'}/>
            <p className='text-[11px]'>The password must be the one entered previously.</p>
...
function FinalRegistration({password, repassword, handlePassword, handleSetpassword, username, handleUsername}) {
  ...
  return (
    <div className=' mt-4'>
      ...
      <div className=' flex flex-row  gap-18 overflow-hidden w-96'>
