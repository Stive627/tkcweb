...
function GalleryProject({setShowGallery, images, clickIndx}) {
    ...
    <div className=' w-full h-full flex justify-center items-center'>
        ...
        {imglength > 0 ?<button onClick={()=>handleClickBack()} className={` w-10 h-10 flex justify-center items-center rounded-full ${currIndx >0 && 'bg-black ' }`}><ArrowBackIosIcon className=' relative left-1' sx={{color:'white'}}/></button>:<p></p>}
        ...
        {imglength > 0 ?<button disabled={currIndx >= imglength -1} onClick={()=> handleClick()} style={{backgroundColor:currIndx === imglength -1 ? 'rgba(217, 217, 217, 1)':'black'}} className=' w-10 h-10 flex justify-center items-center rounded-full'><ArrowForwardIosIcon className=' relative left-0.5' sx={{color:'white'}}/></button>:<p></p>}
        </div>
    </div>
    </div>
  )
