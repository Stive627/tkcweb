...
function GalleryProject({setShowGallery, images, clickIndx}) {
    ...
    <div className=' w-full h-full flex justify-center items-center'>
        ...
        <div className={`h-full ${large? 'w-96':'w-65'}  flex flex-row overflow-hidden`}>
            {images?.map((elt, indx) => <Image ref={(node) => {imgRef.current.set(elt, node); return()=>imgRef.current.delete(elt)}} key={indx} src={elt} width={large? 385 : 265} height={400} alt={`image no${indx}`}/>)}
        </div>
